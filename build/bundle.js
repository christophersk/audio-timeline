...
var project = {
  clips: [{ url: '/GetToDaChoppa.mp3', startTime: 0, track: null }, { url: '/NotATumah.mp3', startTime: .5, track: 1 }],
  // clips: [
  //   { clipId: 1, startTime: 0, track: 1 }
  // ],
  library: [{ id: 1, url: '/NotATumah.mp3' }],
  tracks: [{ id: 1, volume: 100, isMuted: false }],
...
Timeline.prototype.tick = function () {
  var now = context.currentTime - this.playedAt + this.pointer;
  this.time = now;
  // this.pointer = now;

  var timeSubDivide = 60 / this.tempo;
...
Timeline.prototype.checkAndPlayClick = function (timeSubDivide) {
  if (this.time > this.beat * timeSubDivide && this.time > 1) {
    // the this.time > 1 is to let sound buffer load, lazy hack
...
Timeline.prototype.checkAndPlay = function (now) {
  ...
  */
  this.soundClips.forEach(function (soundClip, index) {
    if (now > soundClip.time) {
      // console.log('tempo is', this.tempo)
      // console.dir(soundClip.sound);
      Timeline.playSound(soundClip.sound.buffer, _this.time - soundClip.time);
      _this.soundClips.splice(index, 1);
...
Promise.all(project.clips.map(function (clip) {
  return _axios2.default.get(clip.url, { responseType: 'arraybuffer' }).then(function (res) {
    return res.data;
  }).then(function (responseAudio) {
    return context.decodeAudioData(responseAudio, function (audio) {
